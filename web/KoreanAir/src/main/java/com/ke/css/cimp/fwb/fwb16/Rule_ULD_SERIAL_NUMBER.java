...
final public class Rule_ULD_SERIAL_NUMBER extends Rule
{
  public Rule_ULD_SERIAL_NUMBER(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_ULD_SERIAL_NUMBER parse(ParserContext context)
  {
    context.push("ULD_SERIAL_NUMBER");
    ...
    if (parsed)
    {
        rule = new Rule_ULD_SERIAL_NUMBER(context.text.substring(a0.start, a0.end), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.pop("ULD_SERIAL_NUMBER", parsed);

    return (Rule_ULD_SERIAL_NUMBER)rule;
  }
}
