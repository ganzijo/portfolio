...
final public class Rule_PID extends Rule
{
  public Rule_PID(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_PID parse(ParserContext context)
  {
    context.push("PID");
    ...
    parsed = false;
    {
      ...
      if (parsed)
      {
        ...
        {
          Rule rule = Rule_MId_PID.parse(context);
          if ((f1 = rule != null))
          {
    ...
    if (parsed)
    {
        rule = new Rule_PID(context.text.substring(a0.start, a0.end), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.pop("PID", parsed);

    return (Rule_PID)rule;
  }
}
