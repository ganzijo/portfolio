...
final public class Rule_Sub_Flight_Number extends Rule
{
  public Rule_Sub_Flight_Number(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_Sub_Flight_Number parse(ParserContext context)
  {
    context.push("Sub_Flight_Number");
    ...
    if (parsed)
    {
        rule = new Rule_Sub_Flight_Number(context.text.substring(a0.start, a0.end), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.pop("Sub_Flight_Number", parsed);

    return (Rule_Sub_Flight_Number)rule;
  }
}
