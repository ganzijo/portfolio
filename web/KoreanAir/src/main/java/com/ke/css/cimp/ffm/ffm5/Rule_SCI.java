...
final public class Rule_SCI extends Rule
{
  public Rule_SCI(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_SCI parse(ParserContext context)
  {
    context.push("SCI");
    ...
    parsed = false;
    {
      ...
      if (parsed)
      {
        ...
        {
          Rule rule = Rule_MId_SCI.parse(context);
          if ((f1 = rule != null))
          {
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          Rule rule = Rule_Hid_Customs_Reference.parse(context);
          if ((f1 = rule != null))
          {
            a1.add(rule, context.index);
            c1++;
    ...
    if (parsed)
    {
        rule = new Rule_SCI(context.text.substring(a0.start, a0.end), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.pop("SCI", parsed);

    return (Rule_SCI)rule;
  }
}
