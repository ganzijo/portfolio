...
final public class Rule_Grp_AWB extends Rule
{
  public Rule_Grp_AWB(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_Grp_AWB parse(ParserContext context)
  {
    context.push("Grp_AWB");
    ...
    if (parsed)
    {
        rule = new Rule_Grp_AWB(context.text.substring(a0.start, a0.end), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.pop("Grp_AWB", parsed);

    return (Rule_Grp_AWB)rule;
  }
}
