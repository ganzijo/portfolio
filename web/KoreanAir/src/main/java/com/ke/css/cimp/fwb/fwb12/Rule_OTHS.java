...
final public class Rule_OTHS extends Rule
{
  public Rule_OTHS(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_OTHS parse(ParserContext context)
  {
    context.push("OTHS");
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          ...
          parsed = false;
          {
            ...
            if (parsed)
            {
              ...
              for (int i2 = 0; i2 < 1 && f2; i2++)
              {
                Rule rule = Rule_OTHER_CHARGES.parse(context);
                if ((f2 = rule != null))
                {
                  a2.add(rule, context.index);
                  c2++;
    ...
    if (parsed)
    {
        rule = new Rule_OTHS(context.text.substring(a0.start, a0.end), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.pop("OTHS", parsed);

    return (Rule_OTHS)rule;
  }
}
