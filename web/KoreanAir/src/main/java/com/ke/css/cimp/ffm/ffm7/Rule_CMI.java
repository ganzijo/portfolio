...
final public class Rule_CMI extends Rule
{
  public Rule_CMI(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_CMI parse(ParserContext context)
  {
    context.push("CMI");
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          ...
          parsed = false;
          {
            ...
            if (parsed)
            {
              ...
              {
                Rule rule = Rule_CMIS.parse(context);
                if ((f2 = rule != null))
                {
    ...
    if (parsed)
    {
        rule = new Rule_CMI(context.text.substring(a0.start, a0.end), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.pop("CMI", parsed);

    return (Rule_CMI)rule;
  }
}
