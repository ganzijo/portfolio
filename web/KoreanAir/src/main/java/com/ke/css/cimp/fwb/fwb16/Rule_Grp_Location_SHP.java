...
final public class Rule_Grp_Location_SHP extends Rule
{
  public Rule_Grp_Location_SHP(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_Grp_Location_SHP parse(ParserContext context)
  {
    context.push("Grp_Location_SHP");
              ...
              for (int i2 = 0; i2 < 1 && f2; i2++)
              {
                ...
                parsed = false;
                {
                  ...
                  if (parsed)
                  {
                    ...
                    for (int i3 = 0; i3 < 1 && f3; i3++)
                    {
                      Rule rule = Rule_SHIPPER_PLACE.parse(context);
                      if ((f3 = rule != null))
                      {
                        a3.add(rule, context.index);
                        c3++;
    ...
    if (parsed)
    {
        rule = new Rule_Grp_Location_SHP(context.text.substring(a0.start, a0.end), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.pop("Grp_Location_SHP", parsed);

    return (Rule_Grp_Location_SHP)rule;
  }
}
