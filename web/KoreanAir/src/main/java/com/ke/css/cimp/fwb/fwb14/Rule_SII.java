...
final public class Rule_SII extends Rule
{
  public Rule_SII(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_SII parse(ParserContext context)
  {
    context.push("SII");
    ...
    parsed = false;
    {
      ...
      if (parsed)
      {
        ...
        {
          Rule rule = Rule_MId_SII.parse(context);
          if ((f1 = rule != null))
          {
      ...
      if (parsed)
      {
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          Rule rule = Rule_INCENTIVE_CHARGE_AMOUNT.parse(context);
          if ((f1 = rule != null))
          {
            a1.add(rule, context.index);
            c1++;
              ...
              for (int i2 = 0; i2 < 1 && f2; i2++)
              {
                ...
                parsed = false;
                {
                  ...
                  if (parsed)
                  {
                    ...
                    for (int i3 = 0; i3 < 1 && f3; i3++)
                    {
                      Rule rule = Rule_INCENTIVE_CASS_INDICATOR.parse(context);
                      if ((f3 = rule != null))
                      {
                        a3.add(rule, context.index);
                        c3++;
    ...
    if (parsed)
    {
        rule = new Rule_SII(context.text.substring(a0.start, a0.end), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.pop("SII", parsed);

    return (Rule_SII)rule;
  }
}
