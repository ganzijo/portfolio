...
final public class Rule_ACD extends Rule
{
  public Rule_ACD(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_ACD parse(ParserContext context)
  {
    context.push("ACD");
    ...
    parsed = false;
    {
      ...
      if (parsed)
      {
        ...
        {
          Rule rule = Rule_MId_ACD.parse(context);
          if ((f1 = rule != null))
          {
              ...
              for (int i2 = 0; i2 < 1 && f2; i2++)
              {
                ...
                parsed = false;
                {
                  ...
                  if (parsed)
                  {
                    ...
                    for (int i3 = 0; i3 < 1 && f3; i3++)
                    {
                      Rule rule = Rule_Grp_ACD.parse(context);
                      if ((f3 = rule != null))
                      {
    ...
    if (parsed)
    {
        rule = new Rule_ACD(context.text.substring(a0.start, a0.end), a0.rules);
    }
    else
    {
        context.index = s0;
    ...
    context.pop("ACD", parsed);

    return (Rule_ACD)rule;
  }
}
